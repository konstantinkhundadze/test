...
class Decoder
{
    ...
    protected function __construct($source, $decodeType)
    {
        ...
        switch ($decodeType) {
            ...
            case Json::TYPE_OBJECT:
                ...
                break;
            default:
                throw new InvalidParamException("Unknown decode type '{$decodeType}', please use one of the constants Json::TYPE_*");
        }
    ...
    protected function _decodeObject()
    {
        ...
        while ($tok && $tok != self::RBRACE) {
            if ($tok != self::DATUM || ! is_string($this->tokenValue)) {
                throw new InvalidParamException('Missing key in object encoding: ' . $this->source);
            }
            ...
            if ($tok != self::COLON) {
                throw new InvalidParamException('Missing ":" in object encoding: ' . $this->source);
            }
    ...
    protected function _decodeArray()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                throw new InvalidParamException('Missing "," in array encoding: ' . $this->source);
            }
    ...
    protected function _getNextToken()
    {
                    ...
                    if ($chr == '\\') {
                        ...
                        switch ($chr) {
                            ...
                            case '\'':
                                ...
                                break;
                            default:
                                throw new InvalidParamException("Illegal escape sequence '{$chr}'");
                        }
                    } elseif ($chr == '"') {
                        break;
                    } else {
        ...
        if ($chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9')) {
            if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s', $str, $matches, PREG_OFFSET_CAPTURE, $start) && $matches[0][1] == $start) {
                ...
                if (is_numeric($datum)) {
                    if (preg_match('/^0\d+$/', $datum)) {
                        throw new InvalidParamException("Octal notation not supported by JSON (value: {$datum})");
                    } else {
                        $val  = intval($datum);
                        $fVal = floatval($datum);
                    ...
                    throw new InvalidParamException("Illegal number format: {$datum}");
                }
            ...
            throw new InvalidParamException('Illegal Token');
        }
